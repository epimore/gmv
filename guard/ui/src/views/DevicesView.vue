...
import { ApiError, errorMessage, listDevices, sendPtz, runtimeStatus, startPreview, type DeviceSummary, type RuntimeStatus } from '@/api/client';
...
async function load() { loading.value = true; unavailable.value = false; try { [rows.value, status.value] = await Promise.all([listDevices(), runtimeStatus()]); selected.value = rows.value[0]; } catch (error) { if (error instanceof ApiError && error.status === 501) { unavailable.value = true; rows.value = []; } else ElMessage.error(errorMessage(error, '设备加载失败')); } finally { loading.value = false; } }
async function preview() { const item = selected.value; const channel = item?.channels[0]; if (!item || !channel) return; try { await startPreview(item.device_id, channel, 'ui-' + Date.now()); ElMessage.success('预览已创建'); status.value = await runtimeStatus(); } catch (error) { ElMessage.error(errorMessage(error, '预览失败')); } }
async function ptz() { const item = selected.value; const channel = item?.channels[0]; if (!item || !channel) return; try { await sendPtz(item.device_id, channel); ElMessage.success('PTZ 命令已接受'); status.value = await runtimeStatus(); } catch (error) { ElMessage.error(errorMessage(error, 'PTZ 失败')); } }
onMounted(load);
</script>
