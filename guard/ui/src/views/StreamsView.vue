...
async function load() { loading.value = true; unavailable.value = false; try { rows.value = await listStreams(); } catch (error) { if (error instanceof ApiError && error.status === 501) { unavailable.value = true; rows.value = []; } else ElMessage.error(errorMessage(error, '流加载失败')); } finally { loading.value = false; } }
async function stop(id: string) { try { await stopStream(id); ElMessage.success('流已停止'); await load(); } catch (error) { ElMessage.error(errorMessage(error, '停止失败')); } }
onMounted(load);
</script>
