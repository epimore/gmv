...
export interface RevealedIntegrationCredential { secret: string }
export interface IntegrationMqttConfig { integration_id: string; command_topic: string; result_topic: string; event_topic_prefix: string; updated_at_ms: number }
export type MqttRuntimeApplyState = 'DISABLED' | 'PENDING' | 'APPLYING' | 'CONNECTED' | 'DEGRADED' | 'ROLLING_BACK';
export interface MqttRuntimeConfig { protocol_version: 'v3' | 'v5'; broker: string; port: number; client_id: string; username: string | null; password_configured: boolean; tls: boolean; publish_event_ttl_sec: number; desired_revision: number; active_revision: number | null; config_version: number; apply_state: MqttRuntimeApplyState; last_error_code: string | null; last_error_summary: string | null; last_transition_at_ms: number; updated_by: string; updated_at_ms: number }
export interface IntegrationMqttRuntime { configured: boolean; broker_connected: boolean; config: MqttRuntimeConfig | null; connection_scope: 'deployment'; qos: number; retain: boolean }
export interface BusinessIntegrationState { state: 'unconfigured' | 'ready'; integration: IntegrationInfo | null }
export interface IntegrationMasterKeyState { configured: boolean; key_version: number; created_at_ms: number; updated_by: string; updated_at_ms: number }
export interface SaveBusinessIntegrationPayload { request_id: string; name: string; transport: IntegrationTransport; inbound_enabled: boolean; outbound_enabled: boolean; enabled: boolean; scopes: string[]; expires_at_ms: number | null; expected_config_version: number }
export interface SaveMqttRuntimePayload { request_id: string; protocol_version: 'v3' | 'v5'; broker: string; port: number; client_id: string; username: string | null; password?: string; tls: boolean; publish_event_ttl_sec: number; expected_config_version: number }
...
export type StreamProfileVerification = 'confirmed' | 'unverified' | 'unspecified';
export interface StreamSummary { stream_id: string; device_id: string; channel_id: string; node_id: string; instance_id: string; lease_id: string; route_id: string; endpoint: string; video_codec?: string; audio_codec?: string; mime_codec?: string; source_audio_state?: SourceAudioState; output_audio_mode?: OutputAudioMode; audio_recovery_eligible?: boolean; late_track_watch?: boolean; audio_sample_rate?: number; audio_channels?: number; output_generation?: number; requested_stream_profile?: StreamProfile; effective_stream_profile?: StreamProfile; stream_profile_verification?: StreamProfileVerification; subscription_id?: string; session_node_id?: string; session_instance_id?: string; playback_id?: string; playback_generation?: number; playback_start_time_sec?: number; playback_end_time_sec?: number; state: 'running' | 'stopping' | 'stopped' | 'failed' }
...
export type SourceAudioState = 'not_expected' | 'declared_unobserved' | 'detected_unready' | 'ready' | 'unavailable' | 'failed' | 'unspecified';
export type OutputAudioMode = 'none' | 'silent_placeholder' | 'real' | 'unspecified';
export interface StreamOutputSummary { output_id: string; stream_id: string; output_type: 'flv' | 'hls' | 'll_hls' | 'fmp4'; endpoint: string; state: 'preparing' | 'ready' | 'closed' | 'failed'; video_codec?: string; audio_codec?: string; mime_codec?: string; source_audio_state?: SourceAudioState; output_audio_mode?: OutputAudioMode; audio_recovery_eligible?: boolean; late_track_watch?: boolean; audio_sample_rate?: number; audio_channels?: number; generation?: number }
...
export const getBusinessIntegration = () => request<BusinessIntegrationState>('/integrations/business');
...
export const listAvaiInstallations = () => request<AvaiInstallationInfo[]>('/avai/installations');
...
export interface GbChannelImageListParams { session_node_id: string; start_time_ms?: number; end_time_ms?: number; page?: number; page_size?: number }
...
export interface GbStreamPayload { request_id: string; session_node_id?: string; token?: string; start_time_sec?: number; end_time_sec?: number; playback_id?: string; trans_mode?: MediaTransport; output_type?: string; audio_codec?: 'aac'; stream_profile?: StreamProfile }
...
export const listGbChannelImages = (deviceId: string, channelId: string, params: GbChannelImageListParams) => {
  const query = new URLSearchParams({
    session_node_id: params.session_node_id,
    page: String(params.page || 1),
    page_size: String(params.page_size || 12),
  });
  if (params.start_time_ms) query.set('start_time_ms', String(params.start_time_ms));
  if (params.end_time_ms) query.set('end_time_ms', String(params.end_time_ms));
  return request<GbChannelImagePage>('/gb28181/devices/' + gbPath(deviceId) + '/channels/' + gbPath(channelId) + '/images?' + query);
};
export const issueGbChannelImageAccess = (deviceId: string, channelId: string, imageId: string, sessionNodeId: string, mode: 'inline' | 'attachment' = 'inline') => request<GbChannelImageAccess>('/gb28181/devices/' + gbPath(deviceId) + '/channels/' + gbPath(channelId) + '/images/' + gbPath(imageId) + '/access', { method: 'POST', body: JSON.stringify({ session_node_id: sessionNodeId, mode }) });
export const setGbChannelCover = (deviceId: string, channelId: string, imageId: string, sessionNodeId: string) => request<GbChannelInfo>('/gb28181/devices/' + gbPath(deviceId) + '/channels/' + gbPath(channelId) + '/images/' + gbPath(imageId) + '/cover', { method: 'POST', body: JSON.stringify({ session_node_id: sessionNodeId }) });
