...
export function listActiveStreamMonitor(sessionNodeId: string, query: StreamMonitorQuery, afterStreamId = '', limit = 20): Promise<ActiveStreamMonitorPage> { const params = streamMonitorParams(sessionNodeId, query); params.set('limit', String(limit)); if (afterStreamId) params.set('after_stream_id', afterStreamId); return request<ActiveStreamMonitorPage>('/gb28181/streams?' + params); }
...
export const stopMonitoredStream = (sessionNodeId: string, streamId: string, requestId: string) => request<{ stream_id: string; state: 'stopping' | 'stopped'; session_node_id: string; session_instance_id: string }>('/gb28181/streams/' + encodeURIComponent(streamId) + '/stop', { method: 'POST', body: JSON.stringify({ session_node_id: sessionNodeId, request_id: requestId }) });
