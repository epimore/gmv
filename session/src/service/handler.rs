...
pub async fn end_record(stream_record_info: StreamRecordInfo) -> bool {
    if let Some(path_file_name) = stream_record_info.file_name {
        ...
        let biz_id = path.file_stem().and_then(|s| s.to_str()).unwrap_or("");
        ...
        if let Ok(Some(mut record)) = GmvRecord::query_gmv_record_by_biz_id(biz_id).await {
            let e_ts = record.et.and_local_timezone(Local).unwrap().timestamp();
            let total_secs = record.et.sub(record.st).num_seconds();
            let per = (e_ts - (stream_record_info.timestamp as i64)) * 100 / total_secs;
            ...
            if let Ok(_) = record.update_gmv_record_by_biz_id().await {
                let file_info = GmvFileInfo {
                    ...
                    biz_time: Some(Local.timestamp_opt(stream_record_info.timestamp as i64, 0).unwrap().naive_local()),
                    biz_id: record.biz_id.clone(),
                    file_type: Some(1),
                    file_size: stream_record_info.file_size,
                    file_name: record.biz_id,
                    file_format: Some(extension.to_string()),
...
pub async fn download_info_by_stream_id(stream_id: String, stream_server: String, token: String) -> GlobalResult<StreamRecordInfo> {
    ...
    match conf.node_map.get(&stream_server) {
        None => { return Err(GlobalError::new_biz_error(1100, "stream_server 错误", |msg| error!("{msg}"))); }
        Some(node) => {
...
pub async fn download(play_back_model: PlayBackModel, token: String) -> GlobalResult<String> {
    ...
    //查看是否有任务
    if let Some(_) = entity::GmvRecord::query_gmv_record_run_by_device_id_channel_id(device_id, channel_id).await? {
    ...
    let path = Path::new(&storage_path).join(date_str).to_str().ok_or_else(|| GlobalError::new_sys_error("文件存储路径错误", |msg| error!("{msg}")))?.to_string();

    let (stream_id, node_name) = start_invite_stream(device_id, channel_id, &token, play_type, *st, *et, MediaAction::Download(Download::Mp4(path))).await?;
    general::cache::Cache::stream_map_insert_token(stream_id.clone(), token);
    ...
    while let Some((_, node_name)) = node_sets.pop_first() {
        ...
        if let Ok(true) = callback::call_listen_ssrc(stream_id.clone(), &ssrc, token, stream_node.get_local_ip(), stream_node.get_local_port(), media_action.clone()).await {
            let (res, media_map, from_tag, to_tag) = match play_type {
                ...
                PlayType::Down => {
                    CmdStream::download_invite(device_id, channel_id, &stream_node.get_pub_ip().to_string(), *stream_node.get_pub_port(), StreamMode::Udp, &ssrc, st, et, 4).await?
                }
            };
