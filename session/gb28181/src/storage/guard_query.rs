...
impl GbDeviceView {
    ...
    pub async fn list(
        ...
        let monitor_status = monitor_status_param(monitor_status);
        db::fetch_all_as!(
            Self,
            sql,
    ...
    pub async fn count(registered_only: bool, monitor_status: Option<i64>) -> GlobalResult<u64> {
        let monitor_status = monitor_status_param(monitor_status);
        let row: Option<(i64,)> = db::fetch_optional_as!(
            (i64,),
            "SELECT COUNT(*) FROM gb28181_oauth o LEFT JOIN gb28181_device d ON d.device_id=o.device_id WHERE COALESCE(o.del,0)=0 AND (?=0 OR d.register_time IS NOT NULL) AND (?=-1 OR CASE WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time>CURRENT_TIMESTAMP THEN 1 ELSE 0 END=?)",
            registered_only_param(registered_only),
            monitor_status,
            monitor_status,
        )
    ...
    pub async fn count_by_domain(
        ...
        let row: Option<(i64,)> = db::fetch_optional_as!(
            (i64,),
            "SELECT COUNT(*) FROM gb28181_oauth o LEFT JOIN gb28181_device d ON d.device_id=o.device_id WHERE COALESCE(o.del,0)=0 AND o.domain_id=? AND (?='' OR o.device_id LIKE ?) AND (?='' OR o.alias LIKE ?) AND (?=0 OR d.register_time IS NOT NULL) AND (?=-1 OR CASE WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time>CURRENT_TIMESTAMP THEN 1 ELSE 0 END=?)",
            domain_id,
            device_id,
            &device_id_like,
            device_name,
...
fn monitor_status_param(value: Option<i64>) -> i64 {
    value.unwrap_or(-1)
}
...
const GB_DEVICE_LIST_PAGE_MYSQL: &str = "SELECT \n    o.device_id AS device_id,\n    o.domain_id AS domain_id,\n    o.domain AS domain,\n    CAST(o.longitude AS CHAR) AS longitude,\n    CAST(o.latitude AS CHAR) AS latitude,\n    o.address AS address,\n    o.pwd AS pwd,\n    COALESCE(o.pwd_check,0) AS pwd_check,\n    o.alias AS alias,\n    COALESCE(o.status,1) AS status,\n    COALESCE(o.heartbeat_sec,60) AS heartbeat_sec,\n    COALESCE(o.snapshot_to_mode,0) AS snapshot_to_mode,\n    COALESCE(o.del,0) AS del,\n    o.create_time AS create_time,\n    CAST(o.tenant_id AS CHAR) AS tenant_id,\n    o.sys_org_code AS sys_org_code,\n    o.create_by AS create_by,\n    o.update_by AS update_by,\n    o.update_time AS update_time,\n    CASE\n        WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time > NOW() THEN 1\n        ELSE 0\n    END AS monitor_status,\n    d.device_type AS device_type,\n    d.manufacturer AS manufacturer,\n    d.model AS model,\n    d.firmware AS firmware,\n    d.gb_version AS gb_version,\n    COALESCE(CAST(d.max_camera AS SIGNED),0) AS max_camera,\n    COALESCE(cs.camera_in_count,0) AS camera_in_count,\n    CASE\n        WHEN COALESCE(o.status,1)=0 OR d.online_expire_time IS NULL OR NOW() >= d.online_expire_time THEN COALESCE(cs.camera_in_count,0)\n        ELSE COALESCE(cs.camera_off_count,0)\n    END AS camera_off_count,\n    d.register_time AS register_time\n FROM gb28181_oauth o LEFT JOIN gb28181_device d ON d.device_id=o.device_id LEFT JOIN (SELECT device_id,CAST(COUNT(channel_id) AS SIGNED) AS camera_in_count,CAST(SUM(CASE WHEN status IN ('OFF','OFFLINE') THEN 1 ELSE 0 END) AS SIGNED) AS camera_off_count FROM gb28181_device_channel ch WHERE COALESCE((SELECT rc.resource_kind FROM gb28181_resource_confirmation rc WHERE rc.device_id=ch.device_id AND rc.resource_id=ch.channel_id AND rc.status=1),CASE WHEN SUBSTR(ch.channel_id,11,3) IN ('131','132') THEN 'video' ELSE 'other' END)='video' GROUP BY ch.device_id) cs ON cs.device_id=o.device_id WHERE COALESCE(o.del,0)=0 AND (?=0 OR d.register_time IS NOT NULL) AND (?=-1 OR CASE WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time>CURRENT_TIMESTAMP THEN 1 ELSE 0 END=?) ORDER BY o.device_id LIMIT ? OFFSET ?";
const GB_DEVICE_LIST_PAGE_SQLITE: &str = "SELECT \n    o.device_id AS device_id,\n    o.domain_id AS domain_id,\n    o.domain AS domain,\n    CAST(o.longitude AS TEXT) AS longitude,\n    CAST(o.latitude AS TEXT) AS latitude,\n    o.address AS address,\n    o.pwd AS pwd,\n    COALESCE(o.pwd_check,0) AS pwd_check,\n    o.alias AS alias,\n    COALESCE(o.status,1) AS status,\n    COALESCE(o.heartbeat_sec,60) AS heartbeat_sec,\n    COALESCE(o.snapshot_to_mode,0) AS snapshot_to_mode,\n    COALESCE(o.del,0) AS del,\n    o.create_time AS create_time,\n    CAST(o.tenant_id AS TEXT) AS tenant_id,\n    o.sys_org_code AS sys_org_code,\n    o.create_by AS create_by,\n    o.update_by AS update_by,\n    o.update_time AS update_time,\n    CASE\n        WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time > CURRENT_TIMESTAMP THEN 1\n        ELSE 0\n    END AS monitor_status,\n    d.device_type AS device_type,\n    d.manufacturer AS manufacturer,\n    d.model AS model,\n    d.firmware AS firmware,\n    d.gb_version AS gb_version,\n    COALESCE(d.max_camera,0) AS max_camera,\n    COALESCE(cs.camera_in_count,0) AS camera_in_count,\n    CASE\n        WHEN COALESCE(o.status,1)=0 OR d.online_expire_time IS NULL OR CURRENT_TIMESTAMP >= d.online_expire_time THEN COALESCE(cs.camera_in_count,0)\n        ELSE COALESCE(cs.camera_off_count,0)\n    END AS camera_off_count,\n    d.register_time AS register_time\n FROM gb28181_oauth o LEFT JOIN gb28181_device d ON d.device_id=o.device_id LEFT JOIN (SELECT device_id,COUNT(channel_id) AS camera_in_count,SUM(CASE WHEN status IN ('OFF','OFFLINE') THEN 1 ELSE 0 END) AS camera_off_count FROM gb28181_device_channel ch WHERE COALESCE((SELECT rc.resource_kind FROM gb28181_resource_confirmation rc WHERE rc.device_id=ch.device_id AND rc.resource_id=ch.channel_id AND rc.status=1),CASE WHEN SUBSTR(ch.channel_id,11,3) IN ('131','132') THEN 'video' ELSE 'other' END)='video' GROUP BY ch.device_id) cs ON cs.device_id=o.device_id WHERE COALESCE(o.del,0)=0 AND (?=0 OR d.register_time IS NOT NULL) AND (?=-1 OR CASE WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time>CURRENT_TIMESTAMP THEN 1 ELSE 0 END=?) ORDER BY o.device_id LIMIT ? OFFSET ?";
const GB_DEVICE_LIST_PAGE_BY_DOMAIN_FILTER_MYSQL: &str = "SELECT \n    o.device_id AS device_id,\n    o.domain_id AS domain_id,\n    o.domain AS domain,\n    CAST(o.longitude AS CHAR) AS longitude,\n    CAST(o.latitude AS CHAR) AS latitude,\n    o.address AS address,\n    o.pwd AS pwd,\n    COALESCE(o.pwd_check,0) AS pwd_check,\n    o.alias AS alias,\n    COALESCE(o.status,1) AS status,\n    COALESCE(o.heartbeat_sec,60) AS heartbeat_sec,\n    COALESCE(o.snapshot_to_mode,0) AS snapshot_to_mode,\n    COALESCE(o.del,0) AS del,\n    o.create_time AS create_time,\n    CAST(o.tenant_id AS CHAR) AS tenant_id,\n    o.sys_org_code AS sys_org_code,\n    o.create_by AS create_by,\n    o.update_by AS update_by,\n    o.update_time AS update_time,\n    CASE\n        WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time > NOW() THEN 1\n        ELSE 0\n    END AS monitor_status,\n    d.device_type AS device_type,\n    d.manufacturer AS manufacturer,\n    d.model AS model,\n    d.firmware AS firmware,\n    d.gb_version AS gb_version,\n    COALESCE(CAST(d.max_camera AS SIGNED),0) AS max_camera,\n    COALESCE(cs.camera_in_count,0) AS camera_in_count,\n    CASE\n        WHEN COALESCE(o.status,1)=0 OR d.online_expire_time IS NULL OR NOW() >= d.online_expire_time THEN COALESCE(cs.camera_in_count,0)\n        ELSE COALESCE(cs.camera_off_count,0)\n    END AS camera_off_count,\n    d.register_time AS register_time\n FROM gb28181_oauth o LEFT JOIN gb28181_device d ON d.device_id=o.device_id LEFT JOIN (SELECT device_id,CAST(COUNT(channel_id) AS SIGNED) AS camera_in_count,CAST(SUM(CASE WHEN status IN ('OFF','OFFLINE') THEN 1 ELSE 0 END) AS SIGNED) AS camera_off_count FROM gb28181_device_channel ch WHERE COALESCE((SELECT rc.resource_kind FROM gb28181_resource_confirmation rc WHERE rc.device_id=ch.device_id AND rc.resource_id=ch.channel_id AND rc.status=1),CASE WHEN SUBSTR(ch.channel_id,11,3) IN ('131','132') THEN 'video' ELSE 'other' END)='video' GROUP BY ch.device_id) cs ON cs.device_id=o.device_id WHERE COALESCE(o.del,0)=0 AND o.domain_id=? AND (?='' OR o.device_id LIKE ?) AND (?='' OR o.alias LIKE ?) AND (?=0 OR d.register_time IS NOT NULL) AND (?=-1 OR CASE WHEN COALESCE(o.status,1)=1 AND d.online_expire_time IS NOT NULL AND d.online_expire_time>CURRENT_TIMESTAMP THEN 1 ELSE 0 END=?) ORDER BY o.device_id LIMIT ? OFFSET ?";
...
mod tests {
    ...
    #[test]
    fn device_online_status_filter_precedes_mysql_pagination() {
        let sql = GB_DEVICE_LIST_PAGE_BY_DOMAIN_FILTER_MYSQL;

        let filter = sql.find("d.online_expire_time>CURRENT_TIMESTAMP").unwrap();
        let order = sql.find("ORDER BY o.device_id").unwrap();
        let limit = sql.find("LIMIT ? OFFSET ?").unwrap();
        assert!(filter < order && order < limit);
    }

    #[test]
    fn device_online_status_filter_precedes_sqlite_pagination() {
        let sql = GB_DEVICE_LIST_PAGE_BY_DOMAIN_FILTER_SQLITE;
    ...
    #[test]
    fn missing_device_online_status_uses_all_statuses_sentinel() {
        assert_eq!(monitor_status_param(None), -1);
        assert_eq!(monitor_status_param(Some(0)), 0);
        assert_eq!(monitor_status_param(Some(1)), 1);
    }
}
