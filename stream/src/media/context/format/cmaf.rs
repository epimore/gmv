...
use rsmpeg::ffi::{
    ...
    av_malloc, av_packet_ref, av_packet_rescale_ts, av_packet_unref, av_rescale_q,
    ...
    fn write_packet(&mut self, pkt: &AVPacket, timestamp: u64) {
        unsafe {
            ...
            match self.in_timebase_map.get(&pkt.stream_index) {
                None => {
                    warn!("fMP4 write failed,stream index error: {}", &pkt.stream_index);
                    return;
                }
                Some(in_tb) => {
