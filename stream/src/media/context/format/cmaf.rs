...
impl FmtMuxer for CmafFmp4Context {
    fn init_context(
        ...
        unsafe {
            ...
            let nb = (*in_fmt).nb_streams as usize;

            let mut in_tbs = Vec::with_capacity(nb);
            let mut out_tbs = Vec::with_capacity(nb);
            let mut video_si = -1;
            ...
            for i in 0..demuxer_context.codecpar_list.len() {
                ...
                avcodec_parameters_copy((*out_st).codecpar, demuxer_context.codecpar_list[i]);
            ...
            if ret < 0 {
                return Err(GlobalError::new_sys_error(&format!("FMP4 header write failed: {}", show_ffmpeg_error_msg(ret)), |msg| warn!("{msg}")));
            }
