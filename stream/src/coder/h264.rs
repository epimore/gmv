...
impl ToFrame for H264Context {
    fn parse(&mut self, pkt: Packet, codec_payload: &mut CodecPayload) -> GlobalResult<()> {
        if let Ok(raw_data) = self.h264packet.depacketize(&pkt.payload).hand_log(|msg| warn!("{msg}")) {
            ...
            while curr_offset + nal_data_size_len < raw_data_len {
                ...
                if last_offset > raw_data_len {
                    ...
                    codec_payload.video_payload.2 = pkt.header.timestamp;
                    codec_payload.video_payload.1.push(nal_data);
...
impl H264Context {
    ...
    pub fn init_annexb() -> Self {
        Self { h264packet: H264Packet::default() }
    }
    pub fn init_avc() -> Self {
...
mod test {
    ...
    fn test_sps() {
        ...
        println!("{:?}", H264Context::get_width_height_frame_rate(&Bytes::from(sps.to_vec())));
    ...
    fn test_extract_nal_by_annexb() {
        ...
        let mut vec = Vec::new();
        H264Context::extract_nal_annexb_to_len(&mut vec, bytes_annexb).unwrap();
    ...
    fn test_extract_nal_by_annexb1() {
        ...
        let vec = H264Context::extract_nal_by_annexb1(bytes_annexb);
        vec.iter().map(|iter| println!("{:02x?}", iter.to_vec())).count();
    }
