    ...
    pub fn store(mut self) {
        tokio::spawn(async move {
            match self.run().await {
                Ok(_) => {
                    let info = StreamRecordInfo{ path_file_name: Some(format!("{}/mp4/{}",self.path, self.file_name)),file_size: self.file_size as u64,timestamp: self.ts as u32,};
            ...
            tokio::select! {
                ...
                record_info_tx = self.record_info_event_rx.recv() => {
                    if let Ok(record_info_tx) = record_info_tx {
                       let info = StreamRecordInfo{path_file_name: None,file_size: self.file_size as u64,timestamp: self.ts as u32,};
